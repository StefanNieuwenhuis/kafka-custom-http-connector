...
public class HttpApiClientTest {
    ...
    private static class MockConfig extends AbstractConfig {
        public MockConfig(Map<String, String> orignalProps) {
            super(new ConfigDef()
    ...
    public void setUp(){
        ...
        MockConfig config = new MockConfig(props);
        httpApiClient = new HttpApiClient(config);
        httpApiClient.setHttpClient(mockHttpClient);
    }
    ...
    void executeRequest_withSuccessfulGet_returnsResponseBody() throws IOException, URISyntaxException, ParseException {
        ...
        when(mockHttpClient.execute(any(HttpGet.class), any(HttpClientResponseHandler.class)))
                .thenAnswer(invocation -> {
                    HttpClientResponseHandler<String> httpClientResponseHandler = invocation.getArgument(1);
                    return httpClientResponseHandler.handleResponse(mockResponse);
                });

        String responseBody = httpApiClient.executeRequest("http://example.com", "GET");
    ...
    @Test
    void executeRequest_withAuthBearer_executesSuccessfully() throws Exception {
        Map<String, String> props = Collections.singletonMap("http.auth.bearer", "my-auth-bearer-12345");

        MockConfig config = new MockConfig(props);
        HttpApiClient authHttpApiClient = new HttpApiClient(config);

        Field field = HttpApiClient.class.getDeclaredField("httpClient");
        field.setAccessible(true);
        field.set(authHttpApiClient, mockHttpClient);
        ...
        authHttpApiClient.executeRequest("http://example.com", "GET");
    ...
    @Test
    void executeRequest_withFailedRequest_throwConnectException() throws IOException {
        ...
        assertThatThrownBy(() -> httpApiClient.executeRequest("http://example.com", "GET"))
                .isInstanceOf(ConnectException.class);
    }

    @Test
    void close_withApiClient_closesHttpApiClient() throws IOException {
        httpApiClient.close();
